...
public class MyProcessor extends AbstractProcessor{
    ...
    private AnnotatedClass buildAnnotatedClass(TypeElement typeElement) throws Exception {
    ...
    // 生成 StringUtil 源代码
    private void generate(List<AnnotatedClass> list) throws Exception {
        if (list.size() == 0) {
            return;
        }
    ...
    private void genHelloWorld() throws IOException {
        MethodSpec main = methodBuilder("main").addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(void.class).addParameter(String[].class, "args").addStatement("$T.out.println($S)", System.class, "Hello, JavaPoet!").build();
        TypeSpec helloWorld = classBuilder("HelloWorld").addModifiers(Modifier.PUBLIC, FINAL).addMethod(main).build();
    ...
    private String getPackageName(Elements elements, TypeElement typeElement) throws Exception {
